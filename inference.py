...
parser.add_argument(
    "--wav2lip_batch_size", type=int, help="Batch size for Wav2Lip model(s)", default=128
)
...
def face_detect(images):
    ...
    results = [
        [image[y1:y2, x1:x2], (y1, y2, x1, x2)] for image, (x1, y1, x2, y2) in zip(images, boxes)
    ]
...
def main():
    ...
    mel = audio.melspectrogram(wav)  # (80, length)
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(
        ...
        if i == 0:
            ...
            out = cv2.VideoWriter(
                "temp/result.avi", cv2.VideoWriter_fourcc(*"DIVX"), fps, (frame_w, frame_h)
            )
